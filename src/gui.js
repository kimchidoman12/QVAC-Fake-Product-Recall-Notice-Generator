#!/usr/bin/env node
// QVAC Fake Product Recall Notice Generator — GUI mode.
...
const PORT = process.env.PORT ? Number(process.env.PORT) : 29413;
...
async function main() {
  ...
  const server = http.createServer(async (req, res) => {
    ...
    if (req.method === "POST" && req.url === "/api/recallnotice") {
      try {
        ...
        const result = await generate(modelId, item.trim());
  ...
  server.listen(PORT, () => {
    console.log(`▸ QVAC Fake Product Recall Notice Generator GUI ready at http://localhost:${PORT}`);
  });
